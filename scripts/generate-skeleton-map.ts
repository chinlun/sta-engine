...
/**
 * Generates docs/reference/skeleton-file-map.md from the base Skeleton theme ZIP.
 * 
 * This provides the AI with a complete understanding of:
 * - Every file available in the Skeleton theme
 * - The {% schema %} JSON from each section (settings, blocks, presets)
 * - The JSON content of each template
 */

const BASE_THEME = process.env.BASE_THEME_FILE || 'skeleton-theme.zip';
const zipPath = path.join(process.cwd(), BASE_THEME);
const outputPath = path.join(process.cwd(), 'docs', 'reference', 'skeleton-file-map.md');
...
const lines: string[] = [];
lines.push('# Skeleton Theme File Map');
lines.push('');
lines.push(`> Auto-generated from \`${BASE_THEME}\`. Do not edit manually.`);
lines.push(`> Regenerate with: \`npx tsx scripts/generate-skeleton-map.ts\``);
lines.push('');
...
console.log(`✅ Skeleton file map generated: ${outputPath}`);
